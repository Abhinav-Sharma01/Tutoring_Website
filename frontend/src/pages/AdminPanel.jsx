        ...
        {tab === "overview" && (
          <div style={{ animation: "tp-fade-up 0.5s ease" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20, marginBottom: 36 }}>
              {[
                { label: "Total Users", value: users.length, icon: "👥", color: accent },
                { label: "Students", value: students.length, icon: "🎓", color: "#34d399" },
                { label: "Tutors", value: tutors.length, icon: "📚", color: "#a78bfa" },
                { label: "Total Courses", value: courses.length, icon: "📘", color: "#fbbf24" },
              ].map((card, i) => (
                <div key={i} style={{ background: "rgba(6,14,24,0.7)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: "24px 20px", backdropFilter: "blur(12px)" }}>
        ...
        {tab === "users" && (
          <div style={{ animation: "tp-fade-up 0.5s ease" }}>
            <div style={{ position: "relative", marginBottom: 24 }}>
              ...
              <input className="tp-search" placeholder="Search users by name or email..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
